...
#define HunmanIR_GPIO_PIN       BSP_IO_PORT_04_PIN_03

static inline bsp_io_level_t HunmanIR_GPIO_READ(void) {
    bsp_io_level_t p_pin_value;
    fsp_err_t err = R_IOPORT_PinRead(&g_ioport_ctrl, HunmanIR_GPIO_PIN, &p_pin_value);
    if(err != FSP_SUCCESS)
    {
