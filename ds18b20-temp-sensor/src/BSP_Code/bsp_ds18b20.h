...
#define Module_DQ_PIN   BSP_IO_PORT_04_PIN_08 //DS18B20数据引脚

// 设置DQ输出模式
#define DQ_OUT() { \
    fsp_err_t err = R_IOPORT_PinCfg(&g_ioport_ctrl, Module_DQ_PIN, \
            ((uint32_t) IOPORT_CFG_DRIVE_HIGH \
		| (uint32_t) IOPORT_CFG_PORT_DIRECTION_OUTPUT \
		| (uint32_t) IOPORT_CFG_PORT_OUTPUT_HIGH)); \
    if(err != FSP_SUCCESS) { \
        printf("Out Mode Set Failed!!\r\n"); \
    } \
}
...
#define DQ_IN() { \
    ...
    if(err != FSP_SUCCESS) { \
        printf("In Mode Set Failed!!\r\n"); \
    } \
}

// 获取DQ引脚的电平变化
static inline bsp_io_level_t DQ_GET(void) {
    bsp_io_level_t p_pin_value;
    fsp_err_t err = R_IOPORT_PinRead(&g_ioport_ctrl, Module_DQ_PIN, &p_pin_value);
    if(err != FSP_SUCCESS) {
...
// DQ输出
#define DQ(x)       R_IOPORT_PinWrite(&g_ioport_ctrl, Module_DQ_PIN, x)
